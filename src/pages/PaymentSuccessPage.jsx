...
function PaymentSuccessPage() {
  ...
  const bookingNumber = Math.random().toString(36).substr(2, 9).toUpperCase();
              ...
              <button
                onClick={async () => {
                  try {
                    ...
                    const file = new File([blob], `triply-booking-${bookingNumber}.png`, { type: 'image/png' });
                    
                    if (navigator.canShare && navigator.canShare({ files: [file] })) {
                      await navigator.share({
                        title: `حجز Triply - ${bookingNumber}`,
                        text: `رقم الحجز: ${bookingNumber}\nالوجهة: ${bookingData.destination}\nالتكلفة: ${bookingData.totalCost} ر.س`,
                        files: [file]
                      });
                    } else {
                      // نسخ رابط أو تحميل الصورة
                      const url = canvas.toDataURL();
                      const link = document.createElement('a');
                      link.download = `triply-booking-${bookingNumber}.png`;
                      link.href = url;
                      link.click();
                    }
                  } catch (err) {
